...
internal static class SndtatState
{
    private const KeyCode InteractionKey = KeyCode.P;
    private const KeyCode DropBombKey = KeyCode.L;
    internal const string SettingsLobbyDataKey = "Eights_SnD_Settings";
    internal const string LiveLobbyDataKey = "Eights_SnD_Live";
    internal const float PlantDurationSeconds = SndtatRules.PlantDurationSeconds;
    internal const float DefuseDurationSeconds = SndtatRules.DefuseDurationSeconds;
    internal const float FuseDurationSeconds = SndtatRules.FuseDurationSeconds;
    internal const float BombExplosionDelaySeconds = 2f;
    internal const float BombExplosionRadius = 17.5f;
    internal const float InteractionRadius = SndtatRules.InteractionRadius;
    internal const float PlantSiteRadius = SndtatRules.PlantSiteRadius;
    ...
    internal const int PointsPerRoundWin = SndtatRules.PointsPerRoundWin;

    private static bool SndtatEnabled;
    internal static bool Enabled => GameModeManager.IsActive(GameMode.Countertat)
        ? CountertatState.Enabled
        : SndtatEnabled;
    ...
    internal static SndtatWinReason TakeWinReason { get; private set; }
    internal static int OffensiveTeamId { get; private set; } = 0;
    internal static int DefensiveTeamId => SndtatRules.GetOtherTeamId(OffensiveTeamId);
    ...
    internal static SndtatBombStatus BombStatus { get; private set; }
    ...
    private static int _bombDropperPlayerId = -1;
    ...
    {
        if (GameModeManager.ShouldDeferModeDisable(GameMode.Sndtat, enabled))
        {
            return;
        }

        bool changed = SndtatEnabled != enabled;
        SndtatEnabled = enabled;
        if (changed && GameModeManager.IsActive(GameMode.Sndtat))
        {
            ResetMatchState();
        }
    }
    ...
    {
        ApplySettings(Plugin.SndtatEnabled.Value);
    }
    ...
    internal static void PushSettingsIfHost()
    {
        ...
        ModeLobbyDataSync.Publish(SettingsLobbyDataKey, MyceliumNetwork.LobbyHost,
            GameModeManager.RoundId, revision, Plugin.SndtatEnabled.Value ? "1" : "0",
            "1");
        MyceliumNetwork.RPC(Plugin.SndtatModId,
            nameof(Plugin.SyncSndtatSettings), ReliableType.Reliable,
            MyceliumNetwork.LobbyHost, GameModeManager.RoundId, revision,
            Plugin.SndtatEnabled.Value, true);
    }
    ...
    {
        if (GameModeManager.IsActive(GameMode.Sndtat))
        {
    ...
    internal static void OnPlayerEntered(CSteamID player)
    {
        ...
        MyceliumNetwork.RPCTarget(Plugin.SndtatModId,
            nameof(Plugin.SyncSndtatSettings), player, ReliableType.Reliable,
    ...
    internal static void ResetMatchState()
    {
        ...
        WinnerId = -1;
        TakeWinnerId = -1;
        TakeWinReason = SndtatWinReason.None;
        ...
        DefuserPlayerId = -1;
        BombSiteIndex = -1;
        BombStatus = SndtatBombStatus.Home;
        PlantProgress = 0f;
        DefuseProgress = 0f;
    ...
    {
        if (GameModeManager.IsBombModeActive && MyceliumNetwork.IsHost
            && TeamAssignment.Current.Count == 0)
        {
            TeamAssignment.AssignSndtatRound();
        }
    }

    internal static void OnRoundStarted()
    {
        if (!Enabled || !GameModeManager.IsBombModeActive
        ...
        {
            CountertatState.OnRoundStarted(GameModeManager.RoundId);
        }
    ...
    internal static bool EnsureTeamsAssigned()
    {
        ...
        {
            bool assigned = TeamAssignment.AssignSndtatRound();
            if (assigned)
            {
    ...
    internal static void ServerTick(float deltaTime)
    {
        ...
        if (BombStatus != SndtatBombStatus.Planted)
        {
            ...
            {
                CompleteTake(DefensiveTeamId, SndtatWinReason.TimeExpired);
                return;
            }
        }
    ...
    internal static void OnServerKill(int deadPlayerId, int killerId)
    {
        ...
        {
            BombStatus = SndtatBombStatus.Dropped;
            BombCarrierPlayerId = -1;
            BombPosition = deathPosition;
        }
        if (_bombDropperPlayerId == deadPlayerId)
        {
            _bombDropperPlayerId = -1;
        }
    ...
    internal static void OnPlayerLeft(CSteamID player)
    {
        ...
        if (_bombDropperPlayerId == playerId)
        {
            _bombDropperPlayerId = -1;
        }
    ...
    internal static void HandleBombDropRequest(int playerId, int commandId)
    {
        if (!MyceliumNetwork.IsHost || !NetworkAuthority.IsLocalPlayer(playerId)
            || commandId < 0)
        {
    ...
    internal static void HandleBombDropRequest(int playerId, int commandId, RPCInfo info)
    {
        if (!MyceliumNetwork.IsHost || !NetworkAuthority.IsPlayerSender(info, playerId)
            || !InteractionCommands.TryAccept(info.SenderSteamID, commandId))
        {
    ...
    internal static void PollLocalInput()
    {
        ...
        if (Input.GetKeyDown(DropBombKey)
            && BombStatus == SndtatBombStatus.Carried
            && BombCarrierPlayerId == playerId)
        {
            SendBombDropRequest(playerId);
        }
    ...
    {
        if (ClientInstance.Instance == null || !GameModeManager.IsBombModeActive
            || GameModeManager.Phase != GameModePhase.ActiveRound)
        {
            ReleaseLocalMovementLock();
            return;
    ...
    internal static bool TryGetBombPosition(out Vector3 position)
    {
        ...
        return BombStatus != SndtatBombStatus.Home;
    }
    ...
    {
        if (!GameModeManager.IsBombModeActive || !_roundStarted)
        {
            return true;
        }
        ...
        bool lookingAtBomb)
    {
        ...
        if (playerId == _bombDropperPlayerId)
        {
            if (pressed)
            {
                return;
    ...
    private static void SendInteractionRequest(int playerId, bool pressed, bool lookingAtBomb)
    {
        ...
        MyceliumNetwork.RPC(Plugin.SndtatModId,
            nameof(Plugin.RequestSndtatInteraction), ReliableType.Reliable,
            playerId, commandId, pressed, lookingAtBomb);
    }

    private static void SendBombDropRequest(int playerId)
    {
        int commandId = ++_nextLocalCommandId;
        if (MyceliumNetwork.IsHost)
        {
            HandleBombDropRequest(playerId, commandId);
            return;
        }

        MyceliumNetwork.RPC(Plugin.SndtatModId,
            nameof(Plugin.RequestSndtatBombDrop), ReliableType.Reliable,
            playerId, commandId);
    }

    private static void DropBomb(int playerId)
    {
        if (!Enabled || !GameModeManager.IsBombModeActive
            || GameModeManager.Phase != GameModePhase.ActiveRound || !_roundStarted
            || _takeEnding || !AlivePlayers.Contains(playerId)
            || BombStatus != SndtatBombStatus.Carried
            || BombCarrierPlayerId != playerId
            || !TryGetPlayerPosition(playerId, out Vector3 dropPosition))
        {
            return;
        }
        ...
        BombStatus = SndtatBombStatus.Dropped;
        BombCarrierPlayerId = -1;
        BombPosition = dropPosition;
        CancelPlanting();
        _bombDropperPlayerId = playerId;
        HeldInteractions[playerId] = false;
        LookingAtBomb[playerId] = false;
        BroadcastLiveState();
    }
    ...
    private static void StartTake()
    {
        ...
        TakeId++;
        OffensiveTeamId = GameModeManager.IsActive(GameMode.Countertat)
            ? CountertatState.AboubiTeamId
            : SndtatRules.GetOffensiveTeamId(TakeId);
        TakeWinnerId = -1;
        TakeWinReason = SndtatWinReason.None;
        AlivePlayers.Clear();
        HeldInteractions.Clear();
        ...
        BombCarrierPlayerId = carriers.Count > 0
            ? DistributionRandom.SelectPlayer("SndtatCarrier", carriers)
            : -1;
        BombStatus = BombCarrierPlayerId >= 0
            ? SndtatBombStatus.Carried
            : SndtatBombStatus.Home;
        BombSiteIndex = -1;
        PlantingPlayerId = -1;
        DefuserPlayerId = -1;
        PlantProgress = 0f;
        DefuseProgress = 0f;
        TakeTimeRemaining = SndtatRules.TakeTimeLimitSeconds;
        FuseTimeRemaining = 0f;
    ...
    private static bool ProcessBombInteractions(float elapsed, out bool broadcastImmediately)
    {
        ...
        if (BombStatus == SndtatBombStatus.Dropped)
        {
            foreach (int playerId in AlivePlayers)
            {
                ...
                if (SndtatRules.TryRecoverBomb(BombStatus, true, true,
                    out SndtatBombStatus carriedStatus))
                {
        ...
        if (BombStatus == SndtatBombStatus.Carried)
        {
            ...
            else
            {
                ...
                {
                    SndtatRules.TryStartPlant(BombStatus, true, true, out _);
                    ...
                    changed = true;
                    if (SndtatRules.TryCompletePlant(BombStatus, PlantProgress,
                        out SndtatBombStatus plantedStatus))
                    {
        ...
        if (BombStatus == SndtatBombStatus.Planted)
        {
            ...
            if (DefuserPlayerId < 0)
            {
                foreach (int playerId in AlivePlayers)
                {
                    ...
                    if (SndtatRules.TryStartDefuse(BombStatus, true, true, out _))
                    {
            ...
            if (DefuserPlayerId >= 0)
            {
                ...
                changed = true;
                if (SndtatRules.TryCompleteDefuse(BombStatus, DefuseProgress,
                    out SndtatBombStatus defusedStatus))
                {
                    ...
                    DefuserPlayerId = -1;
                    DefuseProgress = 0f;
                    CompleteTake(DefensiveTeamId, SndtatWinReason.BombDefused);
                    return true;
                }
            }
        }
    ...
    {
        bool teamZeroWiped = SndtatRules.IsTeamWiped(AlivePlayers,
            TeamAssignment.Current, 0);
        bool teamOneWiped = SndtatRules.IsTeamWiped(AlivePlayers,
        ...
        return SndtatRules.CanResolveTeamWipe(BombStatus,
            teamZeroWiped ? 0 : 1, DefensiveTeamId);
    }

    private static void CompleteTake(int winningTeamId, SndtatWinReason reason)
    {
        ...
        AnnounceTakeResult();
        BroadcastLiveState();
        if (SndtatRules.IsMatchWon(Scores[winningTeamId],
            GameModeManager.EffectivePointsToWin))
        {
        ...
        if (Plugin.Instance != null)
        {
            Plugin.Instance.StartCoroutine(StartNextTakeAfterDelay(
                SndtatRules.BetweenTakeDelaySeconds,
    ...
    private static void BeginBombExplosion()
    {
        ...
        FuseTimeRemaining = 0f;
        TakeWinReason = SndtatWinReason.BombExploded;
        ...
        {
            CompleteTake(OffensiveTeamId, SndtatWinReason.BombExploded);
        }
    }
    ...
    private static IEnumerator CompleteBombExplosionAfterDelay(int sessionGeneration, int roundId)
    {
        ...
        if (!SessionState.IsCurrent(sessionGeneration) || GameModeManager.RoundId != roundId
            || !GameModeManager.IsBombModeActive || WinnerId >= 0
    ...
    private static SndtatWinReason GetEliminationWinReason(int winningTeamId)
    {
        return winningTeamId == OffensiveTeamId
            ? SndtatWinReason.DefendersEliminated
            : SndtatWinReason.AttackersEliminated;
    }
    ...
    private static void AnnounceTakeResult()
    {
        ...
        {
            SndtatWinReason.TimeExpired => "Time expired",
            SndtatWinReason.BombExploded => "Bomb exploded",
            SndtatWinReason.BombDefused => "Bomb defused",
            SndtatWinReason.AttackersEliminated => "All attackers eliminated",
            SndtatWinReason.DefendersEliminated => "All defenders eliminated",
            _ => "Round complete"
        };
        string resultText = TakeWinReason == SndtatWinReason.BombExploded
            ? $"<color=#FF5A36><b>Boom! Bomb exploded</b></color>\n"
                + $"<color={teamColorMarkup}><b>{TeamDisplayNames.Get(TakeWinnerId, OffensiveTeamId, GameModeManager.IsActive(GameMode.Countertat))} won the take</b></color>"
            : $"<color={teamColorMarkup}><b>{TeamDisplayNames.Get(TakeWinnerId, OffensiveTeamId, GameModeManager.IsActive(GameMode.Countertat))} "
                + $"won the take</b></color>\n<i>{reason}</i>";
        GameModeHud.BroadcastTakeResult(resultText,
            TakeWinReason == SndtatWinReason.BombExploded ? 4f : 3f);
    ...
    private static bool IsLocalActionCandidate(int playerId)
    {
        ...
        return BombStatus == SndtatBombStatus.Planted
            && IsDefensePlayer(playerId)
            && DefuserPlayerId < 0
            && IsNearPlayer(playerId, BombPosition)
            && IsLocalPlayerLookingAtBomb();
    ...
    private static int FindNearbySite(int playerId)
    {
        ...
        for (int siteIndex = 0; siteIndex < 2; siteIndex++)
        {
            ...
            if (verticalOffset < SndtatRules.PlantSiteMinVerticalOffset
                || verticalOffset > SndtatRules.PlantSiteMaxVerticalOffset)
            {
                continue;
            }
    ...
    private static void BroadcastLiveState()
    {
        ...
        MyceliumNetwork.RPC(Plugin.SndtatModId,
            nameof(Plugin.SyncSndtatLiveState), ReliableType.Reliable,
    ...
    {
        MyceliumNetwork.RPCTarget(Plugin.SndtatModId,
            nameof(Plugin.SyncSndtatLiveState), player, ReliableType.Reliable,
    ...
    private static bool TryParseState(string data)
    {
        ...
        if (offenseTeam < 0 || offenseTeam > 1 || bombStatus < 0 || bombStatus > 3
            ...
            || winReason < (int)SndtatWinReason.None
            || winReason > (int)SndtatWinReason.DefendersEliminated
            || takeTimeRemaining < 0f
            || takeTimeRemaining > SndtatRules.TakeTimeLimitSeconds)
        {
            return false;
        ...
        BombStatus = (SndtatBombStatus)bombStatus;
        ...
        TakeWinReason = (SndtatWinReason)winReason;
    ...
    private static void ApplyLobbySettingsSnapshot()
    {
        if (!ModeLobbyDataSync.TryRead(SettingsLobbyDataKey, 2, out CSteamID hostId,
            ...
            || !Sync.TryAcceptSettingsSnapshot(hostId, roundId, revision,
                ModeLobbyDataSync.Source("sndtat", "settings")))
        {
            return;
        }
    ...
    private static void ApplyLobbyLiveSnapshot()
    {
        ...
        ApplyLiveState(hostId, fields[0], teamCount, fields[2], fields[3], takeId,
            winnerId, roundId, revision, ModeLobbyDataSync.Source("sndtat", "live"));
    }
