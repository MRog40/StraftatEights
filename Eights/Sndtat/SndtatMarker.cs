...
internal static class SndtatMarker
{
    ...
    private static readonly Color BombColor = new(0.28f, 0.28f, 0.28f, 1f);
    private static readonly Color BombOverheadMarkerColor = new(0.38f, 0.38f, 0.38f, 1f);
    ...
    internal static void Update()
    {
        ...
        {
            if (!SndtatState.TryGetSitePosition(siteIndex, out Vector3 position))
            {
    ...
    {
        GameObject root = new($"SndtatSite_{siteIndex}");
        GameObject ring = FloatingObjectiveMarker.CreateRing("BombSiteRing",
            SiteLineColor);
        ring.transform.SetParent(root.transform, false);

        GameObject marker = siteIndex == 0
            ? FloatingObjectiveMarker.CreateOverheadChevron("ChevronMarker", overheadColor)
    ...
    private static Color GetOverheadMarkerColor()
    {
        ...
        {
            if (teamId == SndtatState.OffensiveTeamId)
            {
                return new Color32(220, 45, 45, 235);
            }

            if (teamId == SndtatState.DefensiveTeamId)
            {
    ...
    {
        if (SndtatState.BombStatus == SndtatBombStatus.Home
            || (SndtatState.BombStatus == SndtatBombStatus.Carried
                && !CanSeeCarriedBombMarker())
            || !SndtatState.TryGetBombPosition(out Vector3 position))
        {
            ResetBombAudio();
            SetActive(_bomb, false);
            SetActive(_droppedBombMarker, false);
            return;
        }

        if (_bomb == null || !_bomb)
        {
            _bomb = new GameObject("SndtatBomb");
            _bomb.name = "SndtatBomb";
        ...
        bool carried = SndtatState.BombStatus == SndtatBombStatus.Carried;
        bool dropped = SndtatState.BombStatus == SndtatBombStatus.Dropped;
        _bomb.GetComponent<MeshFilter>()!.sharedMesh = GetBombMesh(carried);
        MeshRenderer bombRenderer = _bomb.GetComponent<MeshRenderer>()!;
        bombRenderer.material.color = BombColor;
        bombRenderer.material.SetInt("_ZTest",
            (int)(carried ? CompareFunction.Always : CompareFunction.LessEqual));
        bombRenderer.material.renderQueue = carried
        ...
        UpdateDroppedBombMarker(position, dropped);
        UpdateElectricArcs(SndtatState.PlantingPlayerId >= 0
            || SndtatState.DefuserPlayerId >= 0);

        if (SndtatState.TakeWinReason == SndtatWinReason.BombExploded)
        {
            if (_lastExplosionTakeId != SndtatState.TakeId)
            {
                _lastExplosionTakeId = SndtatState.TakeId;
        ...
        if (SndtatState.BombStatus != SndtatBombStatus.Planted)
        {
            ResetBombAudio();
            return;
        }
    ...
    private static void UpdateDroppedBombMarker(Vector3 position, bool visible)
    {
        if (!visible)
        {
            SetActive(_droppedBombMarker, false);
        ...
        if (_droppedBombMarker == null || !_droppedBombMarker)
        {
            _droppedBombMarker = new GameObject("SndtatDroppedBombMarker");
            MeshFilter filter = _droppedBombMarker.AddComponent<MeshFilter>();
            filter.sharedMesh = GetBombMesh(true);
            MeshRenderer renderer = _droppedBombMarker.AddComponent<MeshRenderer>();
            ConfigureRenderer(renderer, true);
            renderer.material.color = BombOverheadMarkerColor;
        }

        _droppedBombMarker.transform.SetPositionAndRotation(
            position + Vector3.up * BombMarkerHeight, Quaternion.identity);
        _droppedBombMarker.transform.localScale = Vector3.one * BombMarkerSize;
        _droppedBombMarker.GetComponent<MeshRenderer>()!.material.color =
            BombOverheadMarkerColor;
        _droppedBombMarker.SetActive(true);
    }
    ...
    private static void PlayExplosion(Vector3 position)
    {
        ...
        GameObject burstObject = new("SndtatBombExplosion");
        ...
        GameObject lightObject = new("SndtatBombExplosionLight");
    ...
    private static AudioClip GetBombExplosionClip()
    {
        ...
        _bombExplosionClip = AudioClip.Create("SndtatBombExplosion", sampleCount,
            1, sampleRate, false);
    ...
    private static bool CanSeeCarriedBombMarker()
    {
        if (ClientInstance.Instance == null
            ...
            || !TeamAssignment.TryGetTeamId(SndtatState.BombCarrierPlayerId,
    ...
    private static void Clear()
    {
        ...
        if (_droppedBombMarker != null && _droppedBombMarker)
        {
            Object.Destroy(_droppedBombMarker);
            _droppedBombMarker = null;
        }
    }
}
