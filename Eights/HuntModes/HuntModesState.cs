...
internal static class HuntModesState
{
    internal const float TakeDurationSeconds = 90f;
    internal const float TieBreakDurationSeconds = HuntModesRules.TieBreakDurationSeconds;
    internal const float ServerTickIntervalSeconds = 0.1f;
    internal const int PointsPerTakeWin = HuntModesRules.PointsPerTakeWin;
    ...
    internal static bool IsTanktat => IsActive && Variant.ForceCrouch;
    ...
    private static HuntModeVariantDefinition Variant => GameModeManager.ActiveMode switch
    {
        GameMode.Hunttat => HunttatDefinition.Value,
        GameMode.Tanktat => TanktatDefinition.Value,
        _ => NinjatatDefinition.Value
    };
    ...
    {
        GameMode.Hunttat => Plugin.HunttatEnabled.Value,
        GameMode.Tanktat => Plugin.TanktatEnabled.Value,
        _ => Plugin.NinjatatEnabled.Value
    };
    ...
    internal static void PushSettingsIfHost(HuntModeVariantDefinition variant, bool enabled)
    {
    ...
    internal static bool TryAcceptSettingsSnapshot(HuntModeVariantDefinition variant,
    ...
    internal static void ServerTick(float deltaTime)
    {
        ...
        if (TakeTimeRemaining <= 0f)
        {
            ...
            {
                HuntModesRules.AdvanceTieBreakHold(elapsed, TieBreakController,
    ...
    internal static bool TryGetRoleSpawnPosition(PlayerManager manager, out Vector3 position)
    {
        ...
        int originIndex = HuntModesRules.GetOriginIndex(teamId, TakeId);
    ...
    internal static bool TryGetTieBreakObjective(out HardtatObjective objective)
    {
        objective = null!;
        return GameModeManager.TryGetCurrentMapDefinition(out MapDefinition definition)
            && definition.HardtatObjectives.Count > 0
            && (objective = definition.HardtatObjectives[0]) != null;
    }
    ...
    {
        return HuntModesRules.TryGetTeamWipeWinner(AlivePlayers, TeamAssignment.Current,
            out winningTeamId);
    }
    ...
    {
        teamId = -1;
        if (!TryGetTieBreakObjective(out HardtatObjective objective))
        {
            return false;
        }
    ...
    private static void CompleteTake(int winningTeamId)
    {
        ...
        int nextScore = HuntModesRules.AddTakePoints(score,
    ...
    internal static void ApplyLiveState(HuntModeVariantDefinition variant, CSteamID hostId,
