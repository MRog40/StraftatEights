...
internal static class HuntersMarker
{
    ...
    {
        if (!HuntersState.IsActive
            || GameModeManager.Phase != GameModePhase.ActiveRound
            || !HuntersState.IsTieBreakActive
            || !HuntersState.TryGetTieBreakObjective(out HardpointObjective objective))
        {
            Clear();
            return;
        }
    ...
    {
        int controller = HuntersState.TieBreakController;
        if (controller < 0)
        {
