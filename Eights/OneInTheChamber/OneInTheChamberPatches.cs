...
[HarmonyPatch]
internal static class MeleeWeapon_OneInTheChamberKiller_Patch
{
    ...
    private static bool Prepare()
    {
        ...
        if (target == null)
        {
            Plugin.Logger.LogError("[OneInTheChamber] Could not find the generated MeleeWeapon kill method.");
            return false;
        }
    ...
    private static void Prefix(MeleeWeapon __instance, PlayerHealth enemyHealth)
    {
        ...
        if (attackerId >= 0)
        {
            OneInTheChamberState.RecordMeleeKiller(enemyHealth, attackerId);
