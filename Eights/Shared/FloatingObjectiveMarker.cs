...
internal static class FloatingObjectiveMarker
{
    ...
    internal static GameObject CreateOverheadChevron(string name, Color color)
    {
        GameObject marker = new(name);
        MeshFilter meshFilter = marker.AddComponent<MeshFilter>();
        meshFilter.sharedMesh = CreateOverheadChevronMesh();
        MeshRenderer meshRenderer = marker.AddComponent<MeshRenderer>();
        Shader? shader = Shader.Find("Hidden/Internal-Colored")
            ?? Shader.Find("Unlit/Color")
            ?? Shader.Find("Sprites/Default");
    ...
    private static Mesh CreateOverheadChevronMesh()
    {
        ...
        {
            vertices[index] = new Vector3(outline[index].x, thickness, outline[index].y);
            vertices[index + outline.Length] = new Vector3(outline[index].x, -thickness,
                outline[index].y);
        }
        ...
        Mesh mesh = new() { name = "FloatingObjectiveOverheadChevronMesh" };
        mesh.vertices = vertices;
        mesh.triangles = triangles;
        mesh.RecalculateNormals();
        mesh.RecalculateBounds();
