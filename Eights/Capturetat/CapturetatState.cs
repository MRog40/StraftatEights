...
internal static class CapturetatState
{
    internal const string SettingsLobbyDataKey = "Eights_CTF_Settings";
    internal const string LiveLobbyDataKey = "Eights_CTF_Live";
    internal const float MatchDurationMultiplier = CapturetatRules.MatchDurationMultiplier;
    internal const int CapturePoints = CapturetatRules.CapturePoints;
    ...
    private static readonly CapturetatFlagStatus[] FlagStatuses =
        new CapturetatFlagStatus[2];
    ...
    internal static CapturetatFlagStatus GetFlagStatus(int flagIndex)
    {
        return flagIndex >= 0 && flagIndex < FlagStatuses.Length
            ? FlagStatuses[flagIndex]
            : CapturetatFlagStatus.Home;
    }
    ...
    internal static bool TryGetFlagPosition(int flagIndex, out Vector3 position)
    {
        ...
        if (FlagStatuses[flagIndex] == CapturetatFlagStatus.Carried)
        {
    ...
    {
        if (GameModeManager.ShouldDeferModeDisable(GameMode.Capturetat, enabled))
        {
            return;
        }
    ...
    {
        ApplySettings(Plugin.CapturetatEnabled.Value);
    }
    ...
    internal static void PushSettingsIfHost()
    {
        ...
        ModeLobbyDataSync.Publish(SettingsLobbyDataKey, MyceliumNetwork.LobbyHost,
            GameModeManager.RoundId, revision, Plugin.CapturetatEnabled.Value ? "1" : "0",
            "1");
        MyceliumNetwork.RPC(Plugin.CapturetatModId,
            nameof(Plugin.SyncCapturetatSettings), ReliableType.Reliable,
            MyceliumNetwork.LobbyHost, GameModeManager.RoundId, revision,
            Plugin.CapturetatEnabled.Value, true);
    }
    ...
    internal static void OnPlayerEntered(CSteamID player)
    {
        ...
        MyceliumNetwork.RPCTarget(Plugin.CapturetatModId,
            nameof(Plugin.SyncCapturetatSettings), player, ReliableType.Reliable,
    ...
    internal static void ResetMatchState()
    {
        ...
        {
            FlagStatuses[flagIndex] = CapturetatFlagStatus.Home;
    ...
    internal static void PrepareTeamsForRound()
    {
        if (MyceliumNetwork.IsHost)
        {
            TeamAssignment.AssignCapturetatRound();
        }
    }
    ...
    internal static void EnsureTeamsAssigned()
    {
        ...
        {
            if (TeamAssignment.AssignCapturetatRound())
            {
                BroadcastLiveState();
            }
        ...
        bool suddenDeath, int roundId, int revision, string source = "rpc")
    {
        if (teamCount != 2 || matchTimeRemaining < 0f
            || !TryParseFlags(flagsData, out CapturetatFlagStatus[] statuses,
    ...
    private static void ResetRoundState()
    {
        ...
        Scores[0] = 0;
        Scores[1] = 0;
        MatchTimeRemaining = CapturetatRules.GetMatchDuration(
            GameModeManager.EffectivePointsToWin);
        IsSuddenDeath = false;
        _serverTickAccumulator = 0f;
        _roundCompletionRequested = false;
    ...
    private static void InitializeFlagOwnership()
    {
        if (!GameModeManager.TryGetCurrentMapDefinition(out MapDefinition definition)
            || definition.CapturetatObjectives.Count != 2)
        {
            return;
        }
        ...
        {
            FlagTeamIds[flagIndex] = CapturetatRules.FindNearestTeam(
                ToTeamPoint(definition.CapturetatObjectives[flagIndex]),
                ToTeamPoints(definition.TeamOrigins));
            FlagStatuses[flagIndex] = CapturetatFlagStatus.Home;
            FlagCarriers[flagIndex] = -1;
            FlagPositions[flagIndex] = definition.CapturetatObjectives[flagIndex];
        }
    }
    ...
    private static bool ProcessFlagInteractions()
    {
        ...
        foreach (int playerId in PlayerLookup.GetConnectedPlayerIdsReadOnly())
        {
            ...
            if (carriedFlag >= 0)
            {
                ...
                if (ownFlag >= 0 && FlagStatuses[ownFlag] == CapturetatFlagStatus.Dropped
                ...
                if (ownFlag >= 0 && FlagStatuses[ownFlag] == CapturetatFlagStatus.Home
                    && IsNear(playerPosition, GetHomePosition(ownFlag)))
                {
                    Scores.TryGetValue(teamId, out int previousScore);
                    if (!CapturetatRules.TryAwardCapture(Scores, teamId,
            ...
            if (ownFlagIndex >= 0
                && FlagStatuses[ownFlagIndex] == CapturetatFlagStatus.Dropped
                && IsNear(playerPosition, FlagPositions[ownFlagIndex]))
            {
                if (CapturetatRules.TryReturn(FlagStatuses[ownFlagIndex], true,
                    out CapturetatFlagStatus returnedStatus))
                {
            ...
            if (enemyFlagIndex >= 0 && FlagStatuses[enemyFlagIndex]
                != CapturetatFlagStatus.Carried
                && IsNear(playerPosition, GetFlagWorldPosition(enemyFlagIndex)))
            {
                if (CapturetatRules.TryPickup(FlagStatuses[enemyFlagIndex], true,
                    out CapturetatFlagStatus carriedStatus))
                {
    ...
    {
        if (!CapturetatRules.TryDrop(FlagStatuses[flagIndex],
            out CapturetatFlagStatus droppedStatus))
        {
            return false;
        }
    ...
    private static Vector3 GetHomePosition(int flagIndex)
    {
        return GameModeManager.TryGetCurrentMapDefinition(out MapDefinition definition)
            && definition.CapturetatObjectives.Count == 2
            ? definition.CapturetatObjectives[flagIndex]
            : FlagPositions[flagIndex];
    }

    private static Vector3 GetFlagWorldPosition(int flagIndex)
    {
        return FlagStatuses[flagIndex] == CapturetatFlagStatus.Dropped
            ? FlagPositions[flagIndex]
    ...
    {
        return CapturetatRules.TryResolveTimeoutWinner(Scores, out winningTeamId);
    }
    ...
    private static void BroadcastLiveState()
    {
        ...
        MyceliumNetwork.RPC(Plugin.CapturetatModId,
            nameof(Plugin.SyncCapturetatLiveState), ReliableType.Reliable,
    ...
    {
        MyceliumNetwork.RPCTarget(Plugin.CapturetatModId,
            nameof(Plugin.SyncCapturetatLiveState), player, ReliableType.Reliable,
    ...
    private static bool TryParseFlags(string data,
        out CapturetatFlagStatus[] statuses, out int[] carriers, out int[] teamIds,
        out Vector3[] positions)
    {
        statuses = new CapturetatFlagStatus[2];
        carriers = new int[2];
        teamIds = new int[2];
        ...
        for (int flagIndex = 0; flagIndex < 2; flagIndex++)
        {
            ...
            if (fields.Length != 6 || !int.TryParse(fields[0], out int status)
                ...
                || (status == (int)CapturetatFlagStatus.Carried && carrier < 0)
                || (status != (int)CapturetatFlagStatus.Carried && carrier != -1))
            {
                return false;
            }

            statuses[flagIndex] = (CapturetatFlagStatus)status;
    ...
    private static void ApplyLobbySettingsSnapshot()
    {
        if (!ModeLobbyDataSync.TryRead(SettingsLobbyDataKey, 2, out CSteamID hostId,
            ...
            || !Sync.TryAcceptSettingsSnapshot(hostId, roundId, revision,
                ModeLobbyDataSync.Source("capturetat", "settings")))
        {
            return;
        }
    ...
    private static void ApplyLobbyLiveSnapshot()
    {
        ...
        ApplyLiveState(hostId, fields[0], teamCount, fields[2], fields[3],
            ...
            ModeLobbyDataSync.Source("capturetat", "live"));
    }
