...
internal static class CapturetatMarker
{
    ...
    internal static void Update()
    {
        ...
        {
            if (CapturetatState.GetFlagStatus(flagIndex)
    ...
    {
        int carrierId = CapturetatState.GetFlagCarrier(flagIndex);
        ...
        {
            if (CapturetatState.TryGetFlagPosition(flagIndex, out Vector3 lastKnownPosition))
            {
                if (Markers[flagIndex] == null || !Markers[flagIndex])
                {
                    CreateMarker(flagIndex);
    ...
    {
        GameObject marker = new($"CapturetatMarker_{flagIndex}");
    ...
    {
        float alpha = CapturetatState.GetFlagStatus(flagIndex)
            == CapturetatFlagStatus.Carried ? 1f : 0.85f;
        return TeamColorPolicy.GetRelativeTeamColor(
            CapturetatState.GetFlagTeam(flagIndex), alpha);
    }
    ...
    {
        Mesh mesh = new() { name = "CapturetatRingMesh" };
