...
internal static class HardtatState
{
    internal const string SettingsLobbyDataKey = "Eights_Hardtat_Settings";
    internal const string LiveLobbyDataKey = "Eights_Hardtat_Live";
    internal const float ObjectiveDurationSeconds = 50f;
    internal const float WarningDurationSeconds = HardtatRules.NextObjectiveWarningSeconds;
    ...
    {
        if (GameModeManager.ShouldDeferModeDisable(GameMode.Hardtat, enabled))
        {
            return;
        }
    ...
    {
        ApplySettings(Plugin.HardtatEnabled.Value);
    }
    ...
    internal static void PushSettingsIfHost()
    {
        ...
        ModeLobbyDataSync.Publish(SettingsLobbyDataKey, MyceliumNetwork.LobbyHost,
            GameModeManager.RoundId, revision, Plugin.HardtatEnabled.Value ? "1" : "0",
            "1");
        MyceliumNetwork.RPC(Plugin.HardtatModId, nameof(Plugin.SyncHardtatSettings),
            ReliableType.Reliable, MyceliumNetwork.LobbyHost, GameModeManager.RoundId,
            revision, Plugin.HardtatEnabled.Value, true);
    }
    ...
    internal static void OnPlayerEntered(CSteamID player)
    {
        ...
        MyceliumNetwork.RPCTarget(Plugin.HardtatModId, nameof(Plugin.SyncHardtatSettings),
            player, ReliableType.Reliable, MyceliumNetwork.LobbyHost, GameModeManager.RoundId,
            Sync.SettingsRevision, Plugin.HardtatEnabled.Value, true);
    ...
    {
        if (!Enabled || !GameModeManager.IsActive(GameMode.Hardtat))
        {
            return;
        }
        ...
        {
            Plugin.Logger.LogWarning($"[Hardtat] Round start deferred: teams={TeamAssignment.TeamCount} "
    ...
    {
        if (!Enabled || !MyceliumNetwork.IsHost || !GameModeManager.IsActive(GameMode.Hardtat))
        {
            return;
        }
        ...
        else if (!_roundInitialized && !_loggedAssignmentFailure)
        {
            ...
            Plugin.Logger.LogWarning($"[Hardtat] Team assignment failed: host={MyceliumNetwork.IsHost} "
                + $"players=[{string.Join(",", playerIds)}] "
                + $"active={GameModeManager.IsActive(GameMode.Hardtat)}");
        ...
        if (TeamAssignment.Current.Count == 0
            ...
            || definition.HardtatObjectives.Count == 0)
        {
            return;
        }

        HardtatObjective objective = definition.HardtatObjectives[CurrentObjectiveIndex
            % definition.HardtatObjectives.Count];
        ...
        {
            CurrentObjectiveIndex = HardtatRules.GetNextObjectiveIndex(CurrentObjectiveIndex,
                definition.HardtatObjectives.Count);
    ...
    internal static bool TryGetCurrentObjective(out HardtatObjective objective)
    {
        if (GameModeManager.TryGetCurrentMapDefinition(out MapDefinition definition)
            && definition.HardtatObjectives.Count > 0)
        {
            objective = definition.HardtatObjectives[CurrentObjectiveIndex
                % definition.HardtatObjectives.Count];
            return true;
        }
    ...
    internal static bool TryGetNextObjective(out HardtatObjective objective)
    {
        if (GameModeManager.TryGetCurrentMapDefinition(out MapDefinition definition)
            && definition.HardtatObjectives.Count > 1)
        {
            int nextIndex = HardtatRules.GetNextObjectiveIndex(CurrentObjectiveIndex,
                definition.HardtatObjectives.Count);
            objective = definition.HardtatObjectives[nextIndex];
            return true;
        }
    ...
    private static HashSet<int> GetTeamsOnPoint(HardtatObjective objective,
        out List<int> playersOnPoint)
    {
    ...
    {
        MyceliumNetwork.RPCTarget(Plugin.HardtatModId,
    ...
    private static void ApplyLobbySettingsSnapshot()
    {
        if (!ModeLobbyDataSync.TryRead(SettingsLobbyDataKey, 2, out CSteamID hostId,
            ...
            || !Sync.TryAcceptSettingsSnapshot(hostId, roundId, revision,
                ModeLobbyDataSync.Source("hardtat", "settings")))
        {
            return;
        }
