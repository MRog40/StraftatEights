...
internal static class NifetatState
{
    internal const string SettingsLobbyDataKey = "Eights_Nifetat_Settings";
    internal const string LiveLobbyDataKey = "Eights_Nifetat_Live";
    ...
    {
        if (GameModeManager.ShouldDeferModeDisable(GameMode.Nifetat, enabled))
        {
            return;
        }
    ...
    private static void ApplySettingsFromHostConfig() => ApplySettings(Plugin.NifetatEnabled.Value);
    ...
    internal static void PushSettingsIfHost()
    {
        ...
        ModeLobbyDataSync.Publish(SettingsLobbyDataKey, MyceliumNetwork.LobbyHost,
            GameModeManager.RoundId, revision, Plugin.NifetatEnabled.Value ? "1" : "0");
        MyceliumNetwork.RPC(Plugin.NifetatModId, nameof(Plugin.SyncNifetatSettings), ReliableType.Reliable,
            MyceliumNetwork.LobbyHost, GameModeManager.RoundId, revision, Plugin.NifetatEnabled.Value);
    }
    ...
    internal static void OnPlayerEntered(CSteamID player)
    {
        ...
        MyceliumNetwork.RPCTarget(Plugin.NifetatModId, nameof(Plugin.SyncNifetatSettings), player,
            ReliableType.Reliable, MyceliumNetwork.LobbyHost, GameModeManager.RoundId,
            Sync.SettingsRevision, Plugin.NifetatEnabled.Value);
        MyceliumNetwork.RPCTarget(Plugin.NifetatModId, nameof(Plugin.SyncNifetatLiveState), player,
    ...
    internal static void OnPlayerLeft(CSteamID player)
    {
        ...
        if (playerId >= 0 && Kills.Remove(playerId)
            && GameModeManager.IsActive(GameMode.Nifetat))
        {
            BroadcastLiveState();
        }
    }
    ...
    {
        if (!Enabled || !GameModeManager.IsActive(GameMode.Nifetat)
    ...
    private static void BroadcastLiveState()
    {
        ...
        MyceliumNetwork.RPC(Plugin.NifetatModId, nameof(Plugin.SyncNifetatLiveState), ReliableType.Reliable,
    ...
    private static void ApplyLobbySettingsSnapshot()
    {
        if (!ModeLobbyDataSync.TryRead(SettingsLobbyDataKey, 1, out CSteamID hostId,
            ...
            || !Sync.TryAcceptSettingsSnapshot(hostId, roundId, revision,
                ModeLobbyDataSync.Source("nifetat", "settings")))
        {
            return;
        }
    ...
    private static void ApplyLobbyLiveSnapshot()
    {
        ...
        ApplyLiveState(hostId, fields[0], fields[1], winnerId, roundId, revision,
            ModeLobbyDataSync.Source("nifetat", "live"));
    }
}